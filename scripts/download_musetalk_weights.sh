...
huggingface-cli download TMElyralab/MuseTalk --local-dir "$MUSETALK_ROOT_DIR"


echo "Downloading SD VAE weights to $MODEL_DIR/sd-vae..."
huggingface-cli download stabilityai/sd-vae-ft-mse --local-dir "$MODEL_DIR/sd-vae"


echo "Downloading Whisper weights to $MUSETALK_ROOT_DIR/whisper..."
huggingface-cli download openai/whisper-tiny --local-dir "$MUSETALK_ROOT_DIR/whisper" --include "config.json" "pytorch_model.bin" "preprocessor_config.json"

echo "Downloading DWPose weights to $MUSETALK_ROOT_DIR/dwpose..."
huggingface-cli download yzd-v/DWPose --local-dir "$MUSETALK_ROOT_DIR/dwpose" --include "dw-ll_ucoco_384.pth"

# Download SyncNet weights to syncnet subdirectory
echo "Downloading SyncNet weights to $MUSETALK_ROOT_DIR/syncnet..."
huggingface-cli download ByteDance/LatentSync --local-dir "$MUSETALK_ROOT_DIR/syncnet" --include "latentsync_syncnet.pt"


echo "Downloading Face Parse Bisent weights to $MODEL_DIR/face-parse-bisent..."
huggingface-cli download ManyOtherFunctions/face-parse-bisent --local-dir "$MODEL_DIR/face-parse-bisent" --include "79999_iter.pth" "resnet18-5c106cde.pth"
